...
; ainsi, R5 contiendra 0x80000000 si idx est impair ou §zéro s'il est pair§
        ADDS     r4,r1,r2
; R4 = R1+R2 = idx>>1 + idx&(~1) = offset du triplet §débute§ (ou offset de l'octet de gauche)
...
; R1 = R0<<1 = (idx>>1)<<1 = idx&(~1)
        ADDS     r0,r0,r1
; R0 = idx>>1 + idx&(~1) = idx*1.5 = §début§ de l'offset sur le triplet
        ADDS     r1,r3,r0
...
; R0 = octet de gauche
        LSLS     r0,r0,#4
; R0 = left_byte<<4
; §décaler§ middle_byte dans R2 de 4 bits §à§ droite:
        LSRS     r1,r2,#4
; R1=middle_byte>>4
|L0.98|

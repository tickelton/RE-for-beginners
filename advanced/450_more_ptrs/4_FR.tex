...
Plongeons un peu plus profondément dans le code du \ac{CRT} de Borland C++ 3.1, fichier \IT{c0.asm}:
...
\IT{void*} est un type de données reflétant le fait que c'est un pointeur, mais sur
une valeur d'un type de données inconnu (\IT{void}).
...
(ou un pointeur sur une valeur) et un pointeur \IT{next}.
Pour montrer qu'il n'y a pas de n\oe{}ud suivant, 0 est stocké dans le champ \IT{next}.
...
Avec une sorte de \IT{magic number}? Peut-être -1? Ou peut-être avec un bit additionnel?
...
Le résultat est \IT{0x7578feae} et ne change pas après plusieurs lancements, car user32.dll
...
Appelons \IT{MessageBoxA()} par son adresse:

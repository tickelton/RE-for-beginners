...
; RBX will always be "s"
	call	strlen
; check for str\_len==0 and exit if its so
	test	rax, rax
	je	.L9
	lea	rdx, [rax-1]
; RDX will always contain str\_len-1 value, not str\_len
; so RDX is more like buffer index variable
	...
	cmp	cl, 10
	je	.L4
	cmp	cl, 13              ; \verb|exit if its not '\n' and not '\r'|
	jne	.L9
.L4:
...
.L12:
	test	rdx, rdx
; store zero at address s-1+str\_len-1+1 = s-1+str\_len = s+str\_len-1
	mov	BYTE PTR [rsi+1+rdx], 0  
; check for str\_len-1==0. exit if so.
	je	.L9
	sub	rdx, 1                   ; equivalent to str\_len--
; load next character at address s+str\_len-1
	movzx	ecx, BYTE PTR [rbx+rdx]  
	test	cl, cl                   ; is it zero? exit then
	je	.L9
	cmp	cl, 10                   ; \verb|is it '\n'?|
	je	.L12
	cmp	cl, 13                   ; \verb|is it '\r'?|
	je	.L12
.L9:
; return "s"
	mov	rax, rbx

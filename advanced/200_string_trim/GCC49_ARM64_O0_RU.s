str_trim:
	...
	ldr	x0, [x29,24] ; s
	bl	strlen
	str	x0, [x29,40] ; переменная str\_len в локальном стеке
	b	.L2
; начало главного цикла
.L5:
	ldrb	w0, [x29,39]
; W0=c
	cmp	w0, 13       ; \verb|это '\r'?|
	beq	.L3
	ldrb	w0, [x29,39]
; W0=c
	cmp	w0, 10       ; \verb|это '\n'?|
...
; X1=s
	add	x0, x1, x0
; X0=s+str\_len-1
	strb	wzr, [x0]    ; записать байт на s+str\_len-1
; декремент str\_len:
	ldr	x0, [x29,40]
; X0=str\_len
	sub	x0, x0, #1
; X0=str\_len-1
	str	x0, [x29,40]
; сохранить X0 (или str\_len-1) в локальном стеке
.L2:
	ldr	x0, [x29,40]
; str\_len==0?
	cmp	x0, xzr
...
; X1=s
	add	x0, x1, x0
; X0=s+str\_len-1
; загрузить байт по адресу s+str\_len-1 в W0
	ldrb	w0, [x0]

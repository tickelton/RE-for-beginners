...
|L0.12|
        ...
        BEQ      |L0.20|       
        CMP      r1,#0xa       ; \verb|is loaded byte '\n'?|
        BNE      |L0.38|       ; jump to exit, if no
|L0.20|
        SUBS     r0,r0,#1      ; R0-- or str\_len--
        STRB     r3,[r2,#0x1f] ; store 0 at address R2+0x1F=s+str\_len-0x20+0x1F=s+str\_len-1
|L0.24|
        CMP      r0,#0         ; str\_len==0?
        BEQ      |L0.38|       ; yes? jump to exit
        ADDS     r2,r4,r0      ; R2=R4+R0=s+str\_len
        SUBS     r2,r2,#0x20   ; R2=R2-0x20=s+str\_len-0x20
        LDRB     r1,[r2,#0x1f] ; load byte at address R2+0x1F=s+str\_len-0x20+0x1F=s+str\_len-1 to R1

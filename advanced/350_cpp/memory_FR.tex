...
Allouer un objet \IT{sur la pile}:
...
la fin du \IT{scope}---\ac{SP} est remis à son état (comme au début de la fonction)
et le destructeur de \IT{Class} est appelé.
De la même manière, la mémoire allouée pour une structure en C est désallouée à la
sortie de la fonction.

Allouer un objet \IT{dans le \gls{heap}}:
...
un appel à \IT{malloc()}.
En fait, \IT{new} en C++ est un wrapper pour \IT{malloc()}, et \IT{delete} est un
wrapper pour \IT{free()}.
Puisque le bloc de mémoire a été allouée sur le \gls{heap}, il doit être désalloué
explicitement, en utilisant \IT{delete}.
Le destructeur de classe sera appelé automatiquement juste avant ce moment.

Quelle méthode est la meilleure?
L'allocation \IT{isur la pile} est très rapide, et bon pour les petits, à durée de
vie courte objets, qui seront utilisés seulement dans la fonction courante.

L'allocation \IT{sur le heap} est plus lente, et meilleure pour des objets à longue

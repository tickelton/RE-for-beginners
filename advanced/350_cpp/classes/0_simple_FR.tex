...
Pour \IT{c1} un constructeur par défaut sans argument \TT{??0c@@QAE@XZ} est appelé.
Pour \IT{c2} un autre constructeur \TT{??0c@@QAE@HH@Z} est appelé et deux nombres
...
Le \IT{Name mangling} nous permet d'encoder le nom de la classe + le nom de la méthode +
...
\IT{astuce} que nous avons déjà vu plus tôt: \myref{JMP_instead_of_RET}.
...
Ici, nous voyons un autre style de \IT{name mangling}, spécifique à GNU
...
La \IT{représentation interne} de cette fonction a un seul argument, utilisé comme
...
MSVC et GCC est le style d'encodage des noms de fonctions (\IT{name mangling}) et
...
L'\IT{astuce} du \JMP au lieu de \RET est aussi utilisée ici.

...
Метод \TT{readLine()} вызывается по смещению 11, \emph{reference} на строку (введенную пользователем) 
остается на \ac{TOS}.

По смещению 14, \emph{reference} на строку сохраняется в первом слоте \ac{LVA}.

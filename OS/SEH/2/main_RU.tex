...
\TT{\_EXCEPTION\_REGISTRATION} добавляя указатель на \emph{scope table}
...
Это работа функции \TT{\_except\_handler3} читать другие поля, читать \emph{scope table} и решать, какой обработчик исполнять и когда.\\
\\
\myindex{Wine}
\myindex{ReactOS}
...
\emph{Scope table} расположена в сегменте \TT{CONST} --- действительно, эти поля не будут меняться.
...
\emph{Previous try level} меняется вместе с тем, как исполнение доходит до очередного \TT{try}-блока, либо выходит из него.
...
Указатель на \emph{scope table} теперь \glslink{xoring}{про-XOR-ен} с \gls{security cookie}.
...
Вот значение \emph{cookies}: \TT{Cookie Offset} 

...
Et l'instruction \INS{PUSH} écrit d'abord une valeur sur la pile, \IT{et ensuite}
...
\INS{LDMIA} signifie \IT{Load Multiple Registers Increment address After each transfer}
(charge plusieurs registres incrémente l'adresse après chaque transfert).
\INS{STMDB} signifie \IT{Store Multiple Registers Decrement address Before each transfer}

...
そして、\INS{PUSH}命令は最初に値をスタックに書き込み、\IT{次に} \ac{SP} を4で減算します。
...
\INS{LDMIA} は \IT{Load Multiple Registers Increment address After each transfer}の略です。
\INS{STMDB} は \IT{Store Multiple Registers Decrement address Before each transfer}の略です。

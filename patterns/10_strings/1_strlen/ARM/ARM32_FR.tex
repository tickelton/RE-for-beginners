...
Il y a seulement deux variables locales dans notre fonction: \emph{eos} et \emph{str}.
Dans ce listing, généré par \IDA, nous avons renommé manuellement \emph{var\_8} et
\emph{var\_4} en \emph{eos} et \emph{str}.

La première instruction sauve simplement les valeurs d'entrée dans \emph{str} et \emph{eos}.

Le corps de la boucle démarre au label \emph{loc\_2CB8}.

Les trois première instructions du corps de la boucle (\TT{LDR}, \ADD, \TT{STR})
chargent la valeur de \emph{eos} dans \Reg{0}.
Puis la valeur est \glslink{increment}{incrémentée} et sauvée dans \emph{eos}, qui
...
À la fin de la fonction, la différence entre \emph{eos} et \emph{str} est calculée,
...
Comme le conclut LLVM avec l'optimisation, \emph{eos} et \emph{str} n'ont pas besoin
d'espace dans la pile, et peuvent toujours être stockés dans les registres.

Avant le début du corps de la boucle, \emph{str} est toujours dans \Reg{0}, et
\emph{eos}---dans \Reg{1}.

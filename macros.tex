...
\newcommand{\Arrays}{\RU{Массивы}\EN{Arrays}\PTBR{Matriz}\ES{Matriz}\FR{Tableaux}\JPN{配列}\DE{Arrays}}
...
\newcommand{\BitfieldsChapter}{\RU{Работа с отдельными битами}\EN{Manipulating specific bit(s)}\PTBR{Manipulando bit(s) específicos}\ES{Manipulando bit(s) específicos}\FR{Manipulation de bits spécifiques}\JPN{特定のビットを操作する}\DE{Manipulieren einzelner Bits}}
...
\newcommand{\FPUChapterName}{\RU{Работа с FPU}\EN{Floating-point unit}\PTBR{Unidade de Ponto flutuante}\ES{Unidad de punto flotante}\FR{Unité à virgule flottante}\JPN{フローティングポイントユニット}\DE{Gleitkommaeinheit}}
\newcommand{\MoreAboutStrings}{\RU{Еще кое-что о строках}\EN{More about strings}\PTBRph{}\ESph{}\FR{Plus d'information sur les chaînes}\JPN{文字列に関する加筆}\DE{Mehr über Zeichenketten}}
...
\newcommand{\StructuresChapterName}{\RU{Структуры}\EN{Structures}\PTBR{Estruturas}\ES{Estructuras}\FR{Structures}\JPN{構造体}\DE{Strukturen}}
...
\newcommand{\Loops}{\RU{Циклы}\EN{Loops}\DE{Schleifen}\PTBR{Laços}\ES{Bucles}\FR{Boucles}\JPN{ループ}}
